...
output_file = '/home/pc/Desktop/audio/output/output.json'
...
output_data = {
    "text": "",
    "segments": [],
    "language": "hi"  # Adjust the language as needed
}
    ...
    for filename in os.listdir(audio_folder):
        ...
        if filename.endswith('.mp3') or filename.endswith('.wav'):
            ...
            segment = {
                ...
                "seek": 0,  # Adjust if needed
                "start": 0,  # Adjust the start time for this segment
                "end": 0,  # Adjust the end time for this segment
                "text": "",  # This will be filled with the full transcription text
                "tokens": [],  # Token IDs (if available)
                "temperature": 0.0,  # Adjust if needed
                "avg_logprob": -0.5,  # Adjust based on your model's output
                "compression_ratio": 1.0,  # Adjust if needed
                "no_speech_prob": 0.0,  # Adjust if needed
                "confidence": 0.0,  # Adjust based on your results
                "words": []
            }

            # Add the word-level details to the words field
            ...
            # Fill the segment's text and compute the segment duration
            segment["text"] = ' '.join([entry["word"] for entry in word_times_and_confidence])
            segment["start"] = word_times_and_confidence[0]["start_time"]
            segment["end"] = word_times_and_confidence[-1]["end_time"]
            
            # Add segment to output data
