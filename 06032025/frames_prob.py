...
def calculate_average_probability(probs, start_frame, end_frame):
    """
    Calculate the average probability of the frames between `start_frame` and `end_frame` inclusive.
    
    :param probs: numpy array of shape (1, num_frames, num_classes)
    :param start_frame: start frame index (inclusive)
    :param end_frame: end frame index (inclusive)
    :return: average probability for the given frame range
    """
    # Extract the relevant frames
    selected_frames = probs[0, start_frame:end_frame + 1, :]  # +1 to include end_frame
    # Calculate the average probability across all classes for the selected frames
    avg_probability = np.mean(np.max(selected_frames, axis=-1))  # Max probability per frame, then average
...
with open(output_file, "w") as f:
    for filename in os.listdir(audio_folder):
        if filename.endswith(".mp3") or filename.endswith(".wav"):
            ...
            # Calculate the average probability between frame x and frame y
            x, y = 21, 22  # Example: Start frame = 100, End frame = 200
            avg_prob = calculate_average_probability(wav2vec_probs, x, y)
            f.write(f"Average probability between frame {x} and frame {y}: {avg_prob:.4f}\n")
...
print(f"Transcriptions with timestamps, blank segments, and average probabilities saved to {output_file}")
